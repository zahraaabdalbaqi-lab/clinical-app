...
function buildBodyMap(){
  ...
  allBtn.className = "body-btn active";
...
// 🔥 تحديث التشخيص مباشرة بعد التحاليل
...
setTimeout(() => {
    ...
    showRiskAndCare && showRiskAndCare();
}, 100);

// 🔥 إعادة حساب + تحديث فوري

runDiagnosis();

// تحديث صفحة Matching (النتائج)
if (typeof showMatchingResults === "function") {
    showMatchingResults();
    document.getElementById("labResult").innerHTML =
    document.getElementById("labResult").innerHTML;
}

// تحديث صفحة Lab نفسها
if (typeof updateLabDisplay === "function") {
    updateLabDisplay();
}

// تحديث Risk & Care
if (typeof showRiskAndCare === "function") {
    showRiskAndCare();
}
}
...
function resetSelections(){
  ...
  if(symptoms.length){
    selectRegion(currentRegion || "ALL");
  }
}
...
async function loadDatabaseAuto(){
  ...
  try{
    ...
    status.textContent = "Database loaded automatically ✔️";
